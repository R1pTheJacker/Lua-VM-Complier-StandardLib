...
func (self *luaTable) put(key, val luaValue) {
	...
	if val != nil {
		...
		delete(self._map, key)//remove un necessary nil pair to save space
	}
}
...
func (self *luaTable) _shrinkArray() {
	for i := len(self.arr) - 1; i >= 0; i-- {
		if self.arr[i] == nil {
			self.arr = self.arr[0:i] //truncate nil values
		}
	}
}

func (self *luaTable) _expandArray() {
	for idx := int64(len(self.arr)); true; idx++ {
		if val, found := self._map[idx]; found { //if found val in map, transfer it to array
