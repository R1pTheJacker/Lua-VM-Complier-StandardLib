...
func printOperands(i Instruction) {
	...
	case IABC:
		a, b, c := i.ABC()
		fmt.Printf("%d", a)
		//if the most signaficant bit of b, c is 1, assume it is constant, print as minus number
		if i.BMode() != OpArgN {
			if b > 0xFF {
	...
	case IABx:
		a, bx := i.ABx()
		fmt.Printf("%d", a)
		//if the most signaficant bit of bx is 1, assume it is constant, print as minus number
		if i.BMode() == OpArgK {
			fmt.Printf(" %d", -1-bx)
