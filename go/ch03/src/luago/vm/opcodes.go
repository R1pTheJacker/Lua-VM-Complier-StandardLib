...
//operation number
const (
	OpArgN = iota	//not used
	OpArgU			//used
	OpArgR			//a register or a jump offset
	OpArgK			//a constant or a register/constant
)

//encoding instructions to a struct
type opcode struct {
	testFlag byte		//test operator(before jump operation)
	setAFlag byte		//set register A
	argBMode byte		//argument B mode
	argCMode byte		//argument C mode
	opMode 	 byte		//op mode
	name 	 string
}
